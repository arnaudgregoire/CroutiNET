...
roads_dictionnary_dir = os.path.join(base_dir,"roads_recent_dictionnary.csv")
roads_correct_dictionnary_dir = os.path.join(base_dir,"roads_recent_correct_dictionnary.csv")

img_dir = os.path.join(base_dir,"recent_roads")
...
with open(roads_dictionnary_dir, 'r') as csvfileReader:
    ...
    for row in reader:
        ...
        if(line != []):
            if (checkImage("2017_" + line[4] + "_" + line[2] + '.jpg' )):
