...
"""
Downloads Google StreetView images of random points in Paris
...
# Shapefile with points on roads with bearing of roads and segment grouping variable. There 58631 points in this file
ds = ogr.Open(os.path.join(shp_dir,"roadspointwithbearing.dbf"))

# number of samples for each road segment, e.g., how many points on a road segment do you want to get photos at
nptsSample = 1

# number of subsamples for all grouped road segments, e.g., there are 6150 unique road segments
nsubsample = 6150
...
elist=[row.GetField("n_sq_vo") for row in layer]
...
def download(start_idx, end_idx, api_key):
    with open(os.path.join(base_dir, "roads_dictionnary.csv"), 'a') as f:
        for i in range(start_idx, end_idx) :#range(0,tt.shape[0])
            ...
            rd_segment = feature.GetField("n_sq_vo")
            ...
            if len(panIds) > 0:
                ...
                f.write("{},{},{},{},{}\n".format(rd_segment, lon, lat, heading, panIds[pid]["panoid"]))
...
download(0,nsubsample -1,API_KEY_2)